package ext.flex.spark.layouts
{
   ...
   public class TileLayout extends LayoutBase
   {
      ...
      private var _columnCountMax:uint = 0;
      /**
       * Max number of columns to be displayed. If you set this to positive integer, explicit
       * width of the <code>target</code> is overriden.
       * 
       * @see #target
       */
      public function set columnCountMax(value : uint) : void
      {
         if (_columnCountMax != value)
         {
            _columnCountMax = value;
            invalidateTargetSizeAndDisplayList();
         }
      }
      /**
       * @private
       */
      public function get columnCountMax() : uint
      {
         return _columnCountMax;
      }
      private function get isColumnCountMaxInEffect() : Boolean
      {
         return columnCountMax > 0;
      }
      ...
      private var _rowCountMax:uint = 0;
      /**
       * Max number of rows to be displayed. If you set this to positive integer, explicit
       * height of the <code>target</code> is overriden.
       * 
       * @see #target
       */
      public function set rowCountMax(value : int) : void
      {
         if (_rowCountMax != value)
         {
            _rowCountMax = value;
            invalidateTargetSizeAndDisplayList();
         }
       ...
       * @private
       */
      public function get rowCountMax() : int
      {
         return _rowCountMax;
      }
      private function get isRowCountMaxInEffect() : Boolean
      {
         return rowCountMax > 0;
      }
...
// ### @autor Mikism ### //
         // 
         else if (isColumnCountMaxInEffect || isRowCountMaxInEffect)
         {
            if (isColumnCountMaxInEffect && isRowCountMaxInEffect)
            {
               if (orientation == TileOrientation.ROWS)
               {
                  _columnCount = Math.min(elementCount, columnCountMax);
                  _rowCount = Math.min(Math.ceil(elementCount / _columnCount), rowCountMax);
               }
               else
               {
                  _rowCount = Math.min(elementCount, _rowCountMax);
                  _columnCount = Math.min(Math.ceil(elementCount / _rowCount), columnCountMax);
               }
            }
            else if (isColumnCountMaxInEffect)
            {
               _columnCount = Math.min(elementCount, columnCountMax);
               _rowCount
            }
            ...
            {
               _rowCount = Math.min(elementCount, rowCountMax);
               _columnCount = Math.ceil(elementCount / _rowCount);
            }
         }
// ### @autor Mikism ### //
// ##################### //
