...
describe GameController do
  ...
  describe "not logged in" do
    before(:each) do
      init_controller GameController, :login => false
    end
    ...
    describe "game|config" do
      ...
      it "should have config filtered by regexp (default ruleset)" do
        ...
        response[:config].should equal_to_hash(
          CONFIG.constantize_speed(
            CONFIG.filter(GameController::SENDABLE_RE, 'default')
          )
        )
      end
    end
  ...
  describe "logged in" do
    ...
    describe "game|config" do
      ...
      it "should have config filtered by regexp" do
        ...
        response[:config].should equal_to_hash(
          CONFIG.constantize_speed(
            CONFIG.filter(GameController::SENDABLE_RE, session[:ruleset])
          )
        )
      end
