...
def update_config_template(file)
  config_path = file.sub(/\.template\.yml/, '.yml')
  if File.exists?(config_path)
    contents = File.read(config_path)
    match = contents.match(/^--- # .+? \[(.+?)\]$/)
    hash = match.nil? ? "" : match[1]
  else
    hash = ""
  end

  template_content = File.read(file)
  template_hash = Digest::SHA1.hexdigest(template_content)
  if hash == template_hash
    LOGGER.debug "Not expanding template #{file} because hashes match: #{hash}."
  else
    LOGGER.info "Expanding template #{file} to #{config_path}."
    template = ERB.new(template_content)
    expanded = template.result(binding)
    expanded.sub!(/^---/, "--- # Autogenerated from #{File.basename(file)} [#{
      template_hash}]")
    File.open(config_path, "w") { |f| f.write(expanded) }
  end
end
...
def load_config
  ...
  Dir["#{sets_dir}/*"].each do |dir_name|
    ...
    # sort ensures that templates go first in this list.
    Dir[File.join(dir_name, 'sections', '**', '*.yml')].sort.each do |file|
      if file.ends_with?(".template.yml")
        update_config_template(file)
      else
