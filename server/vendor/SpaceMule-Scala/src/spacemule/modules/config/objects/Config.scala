...
import spacemule.modules.pmg.objects.ss_objects.{RichAsteroid, Asteroid, Planet}
...
object Config {
  ...
  private def range(key: String): Range = {
    val list = get[List[Int]](key)
    return Range.inclusive(list(0), list(1))
  }
  ...
  private def chances(name: String): List[Chance] = list[List[Int]](
    name
  ).map { chanceList => Chance(chanceList(0), chanceList(1)) }
  ...
  def extractorNpcChance(blockTile: BlockTile): Int = blockTile match {
    case BlockTile.Ore => range("npc.planet.tiles.ore.chance").random
    case BlockTile.Geothermal => 
      range("npc.planet.tiles.geothermal.chance").random
    case BlockTile.Zetium => range("npc.planet.tiles.zetium.chance").random
  }

  def getBuildingArea(name: String): Area = area(name.camelcase)
  ...
  def folliageKinds1stType(terrainType: Int) = list[Int](
    "folliage.type.1.variations.%d".format(terrainType)
  )
  ...
  def folliageKinds2ndType(terrainType: Int) = list[Int](
    "folliage.type.2.variations.%d".format(terrainType)
  )
}